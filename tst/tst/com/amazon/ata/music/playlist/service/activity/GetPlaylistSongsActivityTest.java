...
public class GetPlaylistSongsActivityTest {
    ...
    void handleRequest_playlistExistsWithSongs_returnsSongsInPlaylist() {
        // GIVEN
        Playlist playlist = PlaylistTestHelper
                              .generatePlaylistWithNAlbumTracks(3);
        ...
        // WHEN
        GetPlaylistSongsResult result =
          getPlaylistSongsActivity.handleRequest(request, null);

        // THEN
        AlbumTrackTestHelper.assertAlbumTracksEqualSongModels(
          playlist.getSongList(), result.getSongList());
    }

    @Test
    void handleRequest_playlistExistsWithoutSongs_returnsEmptyList() {
        // GIVEN
        Playlist emptyPlaylist = PlaylistTestHelper
                                   .generatePlaylistWithNAlbumTracks(0);
    ...
    void handleRequest_withDefaultSongOrder_returnsDefaultOrderedPlaylistSongs() {
        // GIVEN
        Playlist playlist =
          PlaylistTestHelper.generatePlaylistWithNAlbumTracks(10);
        String playlistId = playlist.getId();
    ...
    void handleRequest_withReversedSongOrder_returnsReversedPlaylistSongs() {
        // GIVEN
        Playlist playlist =
          PlaylistTestHelper.generatePlaylistWithNAlbumTracks(9);
        String playlistId = playlist.getId();
        List<AlbumTrack> reversedAlbumTracks =
          new LinkedList<>(playlist.getSongList());
        ...
        // THEN
        AlbumTrackTestHelper.assertAlbumTracksEqualSongModels(
          reversedAlbumTracks, result.getSongList());
    }

    @Test
    void handleRequest_withShuffledSongOrder_returnsSongsInAnyOrder() {
        Playlist playlist =
          PlaylistTestHelper.generatePlaylistWithNAlbumTracks(8);
        String playlistId = playlist.getId();
        ...
        assertEquals(playlist.getSongList().size(),
                     ...
                     String.format("Expected album tracks (%s) and song models" +
                                     " (%s) to be the same length",
                                   playlist.getSongList(),
                                   result.getSongList()));
        assertTrue(
            songModels.containsAll(result.getSongList()),
            String.format("album list (%s) and song models (%s) are the same" +
                            " length, but don't contain the same " +
